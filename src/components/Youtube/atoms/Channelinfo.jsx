...
const Text = styled.div`
  ...
  -webkit-line-clamp: ${({ isShow }) => (isShow === "더보기" ? 3 : "none")};
  overflow: hidden;
...
const Channelinfo = ({ data, channel }) => {
  const [isShow, setisShow] = useState("더보기");
  const text = data.snippet.localized.description;
  // const text = data.snippet.localized.description.replace(
  //   /(?:\r\n|\r|\n)/g,
  //   <br></br>
  // );
  useEffect(() => {
  ...
  return (
    <Container>
      ...
      <Description>
        <Text isShow={isShow}>{text}</Text>
        <Moretext onClick={() => onClickMore()}>{isShow}</Moretext>
      </Description>
    </Container>
  );
};
